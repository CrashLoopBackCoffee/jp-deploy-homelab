...
This is a homelab infrastructure-as-code project that uses **Pulumi** with **Python** to manage and deploy various services across multiple environments. The project is built on top of **Proxmox** for virtualization and uses **k0s** for the Kubernetes cluster. **Cloudflare** is used for DNS and other services.
...
- **Pets** — long-lived, high-touch systems managed with **Ansible** (or **k0sctl** for k0s): OPNsense, Proxmox, network switches, and the k0s cluster bootstrap.
- **Cattle** — reproducible workloads and services managed with **Pulumi (Python)**: everything running on top of k0s.

Never use OpenTofu or Terraform. Pulumi is the exclusive IaC tool for cattle.
